...
public class ImageCut {
	...
	public static void abscutscale(String srcImageFile, String dirImageFile,
			int x, int y, int destWidth, int destHeight, int targetWidth,
			int targetHeight) {
		try {
			Image img;
			ImageFilter cropFilter;
			...
			if (srcWidth >= destWidth && srcHeight >= destHeight) {
				...
				// ImageIO.write(tag, "JPEG", new File(dirImageFile));// 输出到文件流

				Image image2 = img.getScaledInstance(targetWidth, targetHeight,
						Image.SCALE_DEFAULT);
				BufferedImage tag2 = new BufferedImage(targetWidth,
						targetHeight, BufferedImage.TYPE_INT_RGB);
				Graphics g2 = tag2.getGraphics();
				g2.drawImage(image2, 0, 0, null); // 绘制缩小后的图
				g2.dispose();
				ImageIO.write(tag2, "JPEG", new File(dirImageFile));// 输出到文件流
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
